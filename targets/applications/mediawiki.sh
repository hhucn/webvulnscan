...
tar xfz $TMPDIR/mediawiki.tar.gz -C $APACHE_DIR --transform "s#^mediawiki-[0-9.]*#mediawiki#"
...
mysql -u$MEDIAWIKI_DATABASE_USER -p$MEDIAWIKI_DATABASE_PASSWORD $MEDIAWIKI_DATABASE < $APACHE_DIR/mediawiki/maintenance/tables.sql

cp $SCRIPTDIR/applications/mediawiki.config $APACHE_DIR/mediawiki/LocalSettings.php

chown -R www-data:www-data $APACHE_DIR/mediawiki
...
sed -i -e 's#XXX_SITENAME_XXX#'$MEDIAWIKI_WIKI_NAME'#g' \
	...
	$APACHE_DIR/mediawiki/LocalSettings.php
