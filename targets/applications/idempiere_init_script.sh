...
IDEMPIERE_HOME=XXX_IDEMPIERE_DIR_XXX
IDEMPIEREUSER=XXX_IDEMPIERE_USER_XXX
...
start () {
    ...
    sudo su $IDEMPIEREUSER -c "mkdir -p $IDEMPIERE_HOME/log"

    sudo su $IDEMPIEREUSER -c "cd $IDEMPIERE_HOME;$IDEMPIERE_HOME/idempiere-server.sh &> $LOGFILE &"
    RETVAL=$?
    if [ $RETVAL -eq 0 ] ; then
...
stop () {
    ...
    cd $IDEMPIERE_HOME/utils
    . $ENVFILE 
    echo "Trying to stop with OSGI exit"
    sudo ./stopServer.sh
    sleep 5
    getidempierestatus
    if [ $IDEMPIERESTATUS -ne 0 ] ; then
         log_success_msg "Service stopped with stopServer.sh"
    else
